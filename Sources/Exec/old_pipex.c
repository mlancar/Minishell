...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/04 14:56:23 by malancar          #+#    #+#             */
/*   Updated: 2023/10/04 20:08:35 by malancar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

void	dup_fd(t_pipex *cmd, int fd_in, int fd_out, int fd_other)
{
	//d'abord dup pipe et apres dup redir si y'en a !
	//printf("path[%s], argv[%s], argv[%s]\n", cmd->path, cmd->name[0], cmd->name[1]);
	if (cmd->max == 1)
	{
...
void	first_cmd(t_lst_cmd *argv, t_pipex *cmd)
{
	if (cmd->if_here_doc == 1)
	{
		...
		exec_cmd(argv, cmd, cmd->infile, cmd->fd[1], cmd->fd[0]);
}
...
void	pipex(t_lst_cmd *argv, t_pipex *cmd)
{
	...
	//printf("cmd->index_pid = %d, cmd->max = %d\n", cmd->index_pid, cmd->max);
	...
	while (cmd->index_pid <= cmd->max)
	{
		...
		if (cmd->index_pid == cmd->first)
			...
			exec_cmd(argv, cmd, cmd->fd[0], cmd->outfile, cmd->fd[1]);
		else if ((cmd->index_pid != cmd->first)
			&& (cmd->index_pid != cmd->last))
		{
